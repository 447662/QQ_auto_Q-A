...
def md_inline(text, accent_hex="#CC785C"):
    ...
    def stash_link(match):
        ...
        label = _font_wrap(match.group(1))
        link_placeholders.append(rf'<a href="{match.group(2)}" color="{accent_hex}"><u>{label}</u></a>')
        return f"@@MD_LINK_{idx}@@"
    ...
    text = _font_wrap(text)
    for idx, link in enumerate(link_placeholders):
        text = text.replace(f"@@MD_LINK_{idx}@@", link)
    return text
