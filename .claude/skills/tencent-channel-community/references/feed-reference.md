...
扫描频道帖子或消息时，只允许处理项目已批准口令路线的问题；这是为了避免机器人在群内泛化成通用问答。

**硬性门禁：**

1. 未包含已批准启动口令的帖子/消息，不进行 AI 求助判断、不搜索相关内容、不调用其他技能生成答案。
2. 包含 `#配置推荐` 后，去掉口令再判断剩余内容是否为电脑配置相关问题。允许范围仅包括大学新生购机、笔记本/台式机选购、DIY 装机、硬件兼容性、预算配置、游戏/AI/仿真性能、电商比价或购买渠道；不符合时回复固定文本：`该功能尚未实现，请询问群主或者管理员`。
3. 包含 `#集院问答` 后，去掉口令再判断剩余内容是否为校园问答相关问题。允许范围仅包括新生生活、学业课程、竞赛科研，以及基于已提供资料的校园问答；不符合时只回复范围说明。
4. `#集院问答` 路线必须资料限定：资料不足以确认具体规定时，说明无法确认，并建议查看学院/学校官方通知或咨询辅导员、教务/教学办。
5. 通过门禁后，按对应 skill 的流程生成回答；不要回答对应范围之外的内容。

**推荐路径：**
1. `get-guild-feeds` 或 `get-channel-timeline-feeds` 拉取帖子列表
2. 逐条 `get-feed-detail` 获取完整内容
3. 先执行上述口令和对应范围边界判断
4. 对 `#配置推荐` 问题提取电脑配置关键词，调 `search-guild-feeds` 搜索频道内相关帖子，再按电脑配置推荐技能生成回复
5. 对 `#集院问答` 问题按 campus-qa 技能生成文本回复；如果频道内已有相关资料帖，可先搜索并把资料内容作为回答依据
6. 未通过任何批准门禁时，不进入通用 AI 自动回复
