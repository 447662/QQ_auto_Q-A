...
扫描频道帖子或消息时，只允许处理带有 `#配置推荐` 启动口令的电脑配置相关问题；这是为了避免机器人在群内泛化成通用问答。

**硬性门禁：**

1. 未包含明确启动口令 `#配置推荐` 的帖子/消息，不进行 AI 求助判断、不搜索相关内容、不调用其他技能生成答案，回复固定文本：`该功能尚未实现，请询问群主或者管理员`。
2. 包含 `#配置推荐` 后，去掉口令再判断剩余内容是否为电脑配置相关问题。允许范围仅包括大学新生购机、笔记本/台式机选购、DIY 装机、硬件兼容性、预算配置、游戏/AI/仿真性能、电商比价或购买渠道。
3. 如果剩余内容是学校政策、专业课程、生活咨询、闲聊、广告、群规、其他商品推荐或无法归类为电脑配置问题，回复固定文本：`该功能尚未实现，请询问群主或者管理员`。
4. 通过门禁后，按 `freshman-pc-recommender` 的流程生成电脑配置相关回答；不要回答非电脑配置内容。

**推荐路径：**
1. `get-guild-feeds` 或 `get-channel-timeline-feeds` 拉取帖子列表
2. 逐条 `get-feed-detail` 获取完整内容
3. 先执行上述 `#配置推荐` 口令和电脑配置边界判断
4. 对通过门禁的问题提取电脑配置关键词，调 `search-guild-feeds` 搜索频道内相关帖子
5. 整理相关帖子内容并按电脑配置推荐技能生成回复，调 `do-comment` 发布评论
6. 未通过门禁或无相关内容时发布固定兜底文本：`该功能尚未实现，请询问群主或者管理员`
