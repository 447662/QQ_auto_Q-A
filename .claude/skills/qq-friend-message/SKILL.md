...
description: Windows 本机 QQ 普通好友消息 GUI 自动化。在本项目 QQ/频道等外部消息自动问答中，只能用于发送已通过“#配置推荐”门禁且确为电脑配置相关问题的回复；未带口令或非电脑配置问题统一发送/返回“该功能尚未实现，请询问群主或者管理员”。用于在 QQ 桌面客户端已登录时，按好友昵称或备注名打开普通好友单聊并填入消息；只有用户明确要求直接发送时才发送。适用于用户说“用 QQ 给某个普通好友发消息”“QQ 好友消息”“不是 QQ 频道”的场景。
metadata:
  version: "0.1.0"
...
在本项目的 QQ、QQ群、QQ 频道、帖子、评论、私信或其他外部消息自动问答场景中，本技能只能发送已经通过 `#配置推荐` 门禁且确认为电脑配置相关问题的回复。

- 未包含明确启动口令 `#配置推荐` 时，不要生成自由回答，发送/返回固定文本：`该功能尚未实现，请询问群主或者管理员`。
- 包含口令但问题不是电脑配置相关时，也发送/返回同一句兜底文本。
