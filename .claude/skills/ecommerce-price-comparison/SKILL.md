---
name: 电商价格比较
description: 从京东、淘宝、天猫、拼多多等中国电商平台抓取商品价格并进行比较分析；在本项目 QQ/频道等外部消息自动问答中，只能作为通过“#配置推荐”门禁后的电脑配置推荐辅助能力使用，未带口令或非电脑配置问题统一返回“该功能尚未实现，请询问群主或者管理员”。支持关键词搜索、商品链接分析、价格历史追踪和购买建议。
---
...
- 未包含明确启动口令 `#配置推荐` 时，不要进行价格比较或回答，直接返回：`该功能尚未实现，请询问群主或者管理员`。
- 包含口令但问题不是电脑配置相关时，也直接返回同一句兜底文本。
- 不得因为用户只发了商品名、商品链接、平台名或价格问题，就绕过门禁触发 AI 自动回答。
